...
def wind_shear_expr() -> Parser:
    return map_(
        and_then(
            identifier("WS"),
            char(" "),
            or_(identifier("LDG"), identifier("TKOF")),
            char(" "),
            identifier("RWY"),
            n_or_more(2, digit_expr()),
            zero_or_more(char("L")),
        ),
        tuple,
    )
...
def weather_expr() -> Parser:
    inte = any_(*[identifier(i) for i in ["-", "+", "RE", "VC"]])
    defi = any_(
        *[identifier(i) for i in ["MI", "BC", "PR", "TS", "BL", "SH", "DR", "FZ"]]
    )
    type_ = any_(
        *[
            identifier(i)
            for i in "BR DS DU DZ FC FG FU GR GS HZ IC PE PO PY RA SA SG SN SQ SS UP VA".split(
                " "
            )
        ]
    )
    return map_(
        and_then(zero_or_more(inte), zero_or_more(defi), one_or_more(type_)), tuple
    )
...
if __name__ == "__main__":
    ...
    metar_report = "METAR VHHH 170745Z 16036G55KT 100V220 0600 R25L/0560 FZFG +TSRA SQ BKN015 OVC028CB 02/M01 Q0993 WS LDG RWY25L TEMPO 2000 +RA="
    print(metar_parser(Stream(metar_report, 0)))
    if False:
